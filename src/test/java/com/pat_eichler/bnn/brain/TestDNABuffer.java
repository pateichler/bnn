...
public class TestDNABuffer {
    ...
    void testDNABalancedGrayCode(){
        ...
        DNABuffer buffer = new DNABuffer(new DNA(data, new int[0]));
        for (int i = 0; i < dataSize.length; i++) {
            assertEquals(expectedData[i], buffer.getBalancedGrayCodeBits(dataSize[i]), "DNA not correctly decoded");
        }
    }

    @Test
    void testDNABalancedGrayCodeBounds(){
        byte[] data = new byte[] {2,32, -127, 52, 1, -92, 20, 23, 84, -2};
        DNABuffer buffer = new DNABuffer(new DNA(data, new int[0]));
        for (int i = 0; i < data.length * 4; i++) {
            int val = buffer.getBalancedGrayCodeBits(2);
